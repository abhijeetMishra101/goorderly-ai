// === CONFIGURATION ===
const TEMPLATE_DOC_ID = '1OqWfQaxC_-O9ytlKFqPqMaCGpQl9N4dPhZzmA5cZPM8'; // Replace with your template ID

const JOURNAL_TIME_HOUR = 6;   // 06:00 AM
const JOURNAL_TIME_MINUTE = 0; // on the dot
...
// === ONE-TIME SETUP: rewire triggers to 06:00 + hourly, active starting tomorrow ===
function setupTriggersStartingTomorrow() {
  ...
  // 3) Create the new daily trigger at 06:00
  ...
  Logger.log('Triggers set: daily 06:00 + hourly. Will activate starting ' + _tomorrowYMD());
}
...
function createDailyJournalAndCalendarEvent() {
  ...
  // Create calendar event at 06:00 - 06:30
...
function checkIfJournalFilled() {
  ...
  // Ensure we only check today's journal (created by the 06:00 job)
...
// === WEB APP ENDPOINT: Voice entry logging ===
function doPost(e) {
  const data = JSON.parse(e.postData.contents);
  const entry = data.text;
  const lat = data.lat || null;
  const lng = data.lng || null;
  const context = data.context || ""; // optional tag like "meeting" etc.
