...
class GoogleDriveService {
  ...
  async updateTemplateWithHourlyTable(documentId) {
    ...
    try {
      ...
      // Check if table already exists - if it does, delete it first to recreate properly
      let docToUse = doc;
      const existingTable = doc.data.body.content.find(el => el.table);
      if (existingTable) {
        console.log('[GoogleDriveService] Found existing table, deleting to recreate properly');
        ...
        // Get document again after deletion - this is now our working document
      ...
      // Find and delete old text lines (Time Slot, Task Description, and time slots)
      // Only delete if no table was deleted (text might be outside table)
      if (!existingTable) {
        const deleteStartIndex = bodyText.indexOf('Time Slot', hourlyPlanIndex);
        
        if (deleteStartIndex !== -1) {
          // Find the actual document index for deletion
          let deleteStartDocIndex = 1;
          currentTextIndex = 0;
          ...
          for (const element of docToUse.data.body.content) {
            if (element.paragraph) {
              ...
              if (currentTextIndex + paraText.length > deleteStartIndex) {
                // Found the start of text to delete
                if (element.paragraph.elements && element.paragraph.elements.length > 0) {
                  deleteStartDocIndex = element.paragraph.elements[0].startIndex || element.startIndex || deleteStartDocIndex;
                } else {
                  deleteStartDocIndex = element.startIndex || deleteStartDocIndex;
                }
                break;
              }
              currentTextIndex += paraText.length;
              deleteStartDocIndex = element.endIndex || deleteStartDocIndex;
            }
          }

          // Find end of text to delete (before next section)
          let deleteEndDocIndex = deleteStartDocIndex;
          currentTextIndex = 0;
          ...
          for (const element of docToUse.data.body.content) {
            if (element.paragraph) {
              ...
              if (currentTextIndex + paraText.length > endIndex) {
                // Found the end
                deleteEndDocIndex = element.startIndex || deleteEndDocIndex;
                break;
              }
              currentTextIndex += paraText.length;
              deleteEndDocIndex = element.endIndex || deleteEndDocIndex;
            }
          }
      ...
      // Create AM table first
      let nextInsertIndex = await createAndFillTable(amSlots, currentInsertIndex);
      ...
      await this.docs.documents.batchUpdate({
        ...
        requestBody: {
          requests: [{
            insertText: {
              location: { index: nextInsertIndex },
              text: '\n'
            }
          }]
        }
      ...
      // Wait and get fresh document
      await new Promise(resolve => setTimeout(resolve, 300));
      const docBeforePM = await this.docs.documents.get({ documentId });
      
      // Find insertion point for PM table (after AM table and break)
      let pmInsertIndex = nextInsertIndex + 1;
      for (const element of docBeforePM.data.body.content) {
        if (element.startIndex >= nextInsertIndex && element.paragraph) {
          pmInsertIndex = element.endIndex || pmInsertIndex;
          break;
        }
      }
      
      // Create PM table
      await createAndFillTable(pmSlots, pmInsertIndex);
